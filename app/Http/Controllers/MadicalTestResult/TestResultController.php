...
class TestResultController extends BaseController
{
        ...
        if ($paitent == null) {
            ...
            foreach ($reviews as $review) {
                foreach ($review->MadicalTest as $madicaltest)
                    if ($madicaltest->executor == null && $madicaltest->spec == "تحاليل")
                        $tests[] = $madicaltest;
            }
    ...
    public function update_madical_test_info(Request $request)
    {
        ...
        $validatedData = $request->validate([
            'file' => 'required|file|mimes:jpg,jpeg,bmp,png,doc,docx,
            csv,rtf,xlsx,xls,txt,pdf,zip,rar',
        ]);
        $new_file = new File();
    ...
    public function store_doc(Request $request)
    {
        ...
        $new_file->name = $name;
        // $FILE = $request->file;
        // Storage::disk('local')->put('public/MadicalTests/' . $name,  $FILE);
